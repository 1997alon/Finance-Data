...
def calc_annualized_return(portfolio_values):
    days_in_year = 365
    daily_trading_30m = 48
    yearly_trading_30m = days_in_year * daily_trading_30m 
    portfolio_trading_days = portfolio_values.shape[0]
    portfolio_trading_years = portfolio_trading_days / yearly_trading_30m 
...
def calc_annualized_sharpe(portfolio_values: pd.Series, rf: float=0.0):
    ...
    annualized_std = portfolio_values.pct_change().std() * np.sqrt(yearly_trading_30m)
...
def calc_sortino(portfolio_values, rf=0.0):
    ...
    daily_trading_30m = 48
    yearly_trading_30m = days_in_year * daily_trading_30m    
    down_deviation = calc_downside_deviation(portfolio_values) * np.sqrt(yearly_trading_30m)
    annualized_return = calc_annualized_return(portfolio_values)
...
def evaluate_strategy(b_df, strat_name):
    total_return = calc_total_return(b_df['portfolio_value'][START_DATE_INDEX:])
    annualized_return = calc_annualized_return(b_df['portfolio_value'][START_DATE_INDEX:])
    annualized_sharpe = calc_annualized_sharpe(b_df['portfolio_value'][START_DATE_INDEX:])
    sortino_ratio = calc_sortino(b_df['portfolio_value'][START_DATE_INDEX:])
    max_drawdown = calc_max_drawdown(b_df['portfolio_value'][START_DATE_INDEX:])
